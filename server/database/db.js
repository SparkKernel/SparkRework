...
const execute = (sql, params) => {
    return new Promise((resolve, reject) => connection.query(sql, params, (err, result) => {if (err) reject(err); resolve(result)}))
}
...
global.exports('query', (sql, params, cb) => {
    ...
    execute(sql, params).then((result) => invoke(cb, result)).catch((err) => invoke(cb, err))
})
